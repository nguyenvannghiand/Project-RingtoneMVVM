    ...
    private fun calculateTax(list: List<Basket>) {
        ...
        for (basket: Basket in list) {
            ...
            if (basket.categoryId == 3) {
                salesTax = AppUtils.getSalesTax(price)
            }

            if (basket.isImported) salesTax += AppUtils.getImportDuty(price)
            totalPrice += price + salesTax
            totalSalesTax += salesTax
            basket.price = AppUtils.formatDecimal(price + salesTax)
        }
        mViewModel.totalPrice.set(AppUtils.formatDecimal(totalPrice))
        mViewModel.salesTax.set(AppUtils.formatDecimal(totalSalesTax))
    }
